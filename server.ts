...
function finalizeSpecificPlayerAuction(player: Player) {
  if (player.currentBid) {
    const winner = leagueState.users.find((u) => u.id === player.currentBid!.userId);
    if (winner) {
      winner.budget -= player.currentBid.amount;
      winner.spent += player.currentBid.amount;

      player.status = 'SOLD';
      player.currentPrice = player.currentBid.amount;
      player.soldTo = {
        userId: winner.id,
        userName: winner.name,
        teamName: winner.teamName,
        amount: player.currentBid.amount,
      ...
      broadcast({
        ...
        data: {
          winner,
          player,
          finalPrice: player.currentBid.amount
        }
      });
      ...
      broadcast({
        type: 'CHAT_NOTIFICATION',
        data: {
          message: `🔨 MARTELO BATIDO! ${player.name} foi arrematado por ${winner.name} (${winner.teamName}) por € ${(player.currentBid.amount / 1000000).toFixed(1)}M!`,
          timestamp: Date.now(),
          type: 'hammer'
        }
      });
...
setInterval(() => {
  ...
  if (leagueState.auction.status === 'ACTIVE') {
    ...
    for (const p of leagueState.players) {
      if (p.status === 'IN_AUCTION' && typeof p.timerRemaining === 'number' && p.timerRemaining > 0) {
        p.timerRemaining -= 1;
        stateChanged = true;
...
async function startServer() {
  ...
  app.post('/api/auction/nominate', authenticate, requireAuth, (req: AuthenticatedRequest, res: Response) => {
    ...
    broadcast({
      type: 'CHAT_NOTIFICATION',
      data: {
        message: `📢 Proposta aberta por ${player.name} (${player.position} - ${player.club}) no valor de € ${(openingAmount / 1000000).toFixed(1)}M (${isAnonymous ? 'Sigilo de Lances' : user.teamName})! Disputa ativa por 24 horas.`,
        timestamp: Date.now(),
        type: 'info'
      }
    });
  ...
  app.post('/api/auction/bid', bidLimiter, authenticate, requireAuth, (req: AuthenticatedRequest, res: Response) => {
    ...
    if (player.currentBid && player.currentBid.userId === user.id) {
      res.status(400).json({ success: false, error: 'Sua equipe já possui a maior proposta ativa por este atleta!' });
      return;
    }
    ...
    const bidNotification = isAnonymous
      ? `💰 Um clube concorrente (Sigilo de Lances) cobriu a proposta por ${player.name} para € ${(bidAmount / 1000000).toFixed(1)}M!`
      : `💰 ${user.name} (${user.teamName}) cobriu a proposta por ${player.name} para € ${(bidAmount / 1000000).toFixed(1)}M!`;
